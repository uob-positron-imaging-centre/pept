...
class ParallelScreensArray(LineData):
    '''A subclass of `LineData` that initialises PEPT data arrays for parallel screens detectors.

    Provides the same functionality as the `LineData` class while
    initialising `line_data` from a given array. This is a helper class
    for **PEPT scanners with two parallel screens**.

    `ParallelScreens` can initialise data from a numpy array or equivalent.
    **The expected data row in the file is `[time, x1, y1, x2, y2]`**. This will
    then be automatically transformed into the standard `line_data` format
    with every row being `[time, x1, y1, z1, x2, y2, z2]`, where `z1 = 0`
    and `z2 = separation`.
    ...
    Parameters
    ----------
    data_array : (N, 5) numpy.ndarray
        An (N, 5) numpy array that stores the PEPT LoRs in the parallel
        screens data format, in which every row is `[time, x1, y1, x2, y2]`.
    sample_size : int, optional
        An `int`` that defines the number of lines that should be
        returned when iterating over `line_data`. A `sample_size` of 0
        yields all the data as one single sample. (Default is 200)
    ...
    def __init__(
        ...
        # Verify that data_array has shape (N, 5)
        if data_array.ndim != 2 or data_array.shape[1] != 5:
            raise ValueError('\n[ERROR]: data_array should have dimensions (N, 5). Received {}\n'.format(data_array.shape))

        number_of_lines = len(data_array)

        # Add Z1 and Z2 columns => [time, X1, Y1, Z1, X2, Y2, Z2]
        # Z1 = 0
        data_array = np.insert(data_array, 3, np.zeros(number_of_lines), axis = 1)
        # Z2 = `separation`
        data_array = np.append(data_array, separation * np.ones((number_of_lines, 1)), axis = 1)
